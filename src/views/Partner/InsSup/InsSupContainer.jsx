...
import InsSupView from './InsSupView'
...
//-- Khai báo cac biến của một hàm Supplier ở đây
    ...
    subcr_ClientReqRcv = socket_sv.event_ClientReqRcv.subscribe((message) => {
        if (message) {
            ...
            switch (reqInfoMap.reqFunct) {
                case glb_sv.supplier_row_FcntNm:
                    console.log('resultGetRowSupplier', props)
                    resultGetRowSupplier(props, message, cltSeqResult, reqInfoMap)
                    break
                case glb_sv.supplier_ins_FcntNm:
                    resultSubmit(props, message, cltSeqResult, reqInfoMap)
                    break
                case glb_sv.supplier_edit_FcntNm:
                    resultSubmit(props, message, cltSeqResult, reqInfoMap)
                    break
...
const getRowSupplier = (props, id) => {}

const resultGetRowSupplier = (props, message = {}, cltSeqResult = 0, reqInfoMap = new requestInfo()) => {}
...
const enhance = compose(
    ...
    lifecycle({
        componentDidMount() {
            if (+this.props.id > 0) {
                getRowSupplier(this.props, this.props.id)
            } else {
...
export default withTranslation()(enhance(InsSupView))
