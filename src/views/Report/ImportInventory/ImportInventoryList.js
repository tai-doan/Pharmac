...
const ImportInventoryList = () => {
    ...
    useEffect(() => {
        getList(searchModal.start_dt, searchModal.end_dt, searchModal.invoice_no, searchModal.invoice_status, searchModal.product_id, glb_sv.defaultValueSearch, glb_sv.defaultValueSearch);
    }, [])
    ...
    const handleResultGetAll = (reqInfoMap, message) => {
        ...
        if (message['PROC_STATUS'] !== 1) {
            ...
            if (newData.rows.length > 0) {
                if (reqInfoMap.inputParam[5] === glb_sv.defaultValueSearch && reqInfoMap.inputParam[6] === glb_sv.defaultValueSearch) {
