...
import { settlementDefaulModal } from './Modal/ExportRepay.modal'
...
const ExportRepayEdit = ({ id, shouldOpenModal, setShouldOpenModal, onRefresh }) => {
    ...
    useHotkeys('esc', () => { setShouldOpenModal(false); setSettlement(settlementDefaulModal) }, { enableOnTags: ['INPUT', 'SELECT', 'TEXTAREA'] })
        ...
        const settlementSub = socket_sv.event_ClientReqRcv.subscribe(msg => {
            if (msg) {
                ...
                switch (reqInfoMap.reqFunct) {
                    case reqFunction.SETTLEMENT_EXPORT_REPAY_UPDATE:
                        resultUpdate(msg, cltSeqResult, reqInfoMap)
                        break
                    case reqFunction.SETTLEMENT_EXPORT_REPAY_BY_ID:
    ...
    return (
        ...
        >
            <Card>
                <CardHeader title={t('settlement.titleEditExport')} />
                <CardContent>
...
export default ExportRepayEdit